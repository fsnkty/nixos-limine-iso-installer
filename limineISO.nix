...
}:
{
  ...
  in
  {
    ...
    fileSystems = {
      ...
      "/iso" = {
        device = "/dev/disk/by-label/${cfg.volumeId}";
        fsType = "iso9660";
        options = [ "ro" ];
    ...
    system.build.isoImage =
      ...
      pkgs.stdenv.mkDerivation {
        ...
        buildCommand = ''
          ...
          # Force mtools to handle long filenames and preserve case
          cat << EOF > mtools.conf
          mtools_skip_check=1
          mtools_lower_case=1
          EOF
          ...
          cp ${pkgs.limine-full}/share/limine/limine-uefi-cd.bin iso_root/boot/limine/
          ...
          limine bios-install $out/${cfg.isoName}
        '';
      };
  };
}
